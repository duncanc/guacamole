...
local zlib1 = ffi.load('zlib1')
local ZLIB_VERSION = '1.2.3';
local sizeof_z_stream = ffi.sizeof('z_stream')

return setmetatable({
	ZLIB_VERSION = ZLIB_VERSION;
	deflateInit = function(strm, level)
		return zlib1.deflateInit_(strm, level, ZLIB_VERSION, sizeof_z_stream)
	end;
	inflateInit = function(strm)
		return zlib1.inflateInit_(strm, ZLIB_VERSION, sizeof_z_stream)
	end;
	deflateInit2 = function(strm, level, method, windowBits, memLevel, strategy)
		return zlib1.deflateInit2_(strm, level, method, windowBits, memLevel, strategy, ZLIB_VERSION, sizeof_z_stream)
	end;
	inflateInit2 = function(strm, windowBits)
		return zlib1.inflateInit2_(strm, windowBits, window, ZLIB_VERSION, sizeof_z_stream)
	end;
	inflateBackInit = function(strm, windowBits, window)
		return zlib1.inflateBackInit_(strm, windowBits, window, ZLIB_VERSION, sizeof_z_stream)
	end;
	z_stream = ffi.typeof 'z_stream';
	gz_header = ffi.typeof 'gz_header';
}, {
	__index = function(self, k)  return zlib1[k];  end;
})
